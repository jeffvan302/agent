...
std::string BuildWebFormattingContext(const std::string& server_address,
    ...
    std::string context =
        ...
        "- When the user should copy a block exactly, use a fenced code block tagged `copy`. "
        "Only `copy` blocks receive a copy button in the web chat; use normal language-tagged "
        "code blocks for examples or code that is only meant to be read.\n"
        "- Put each independently copyable value or command sequence in its own `copy` block.\n"
